...
SYSTEM_DESCRIPTION = "PEMA supports the metabarcoding analysis of four marker genes, 16S rRNA (Bacteria), ITS (Fungi) as well as COI and 18S rRNA (metazoa). As input, PEMA accepts .fastq.gz files as returned by Illumina sequencing platforms."
...
PROFILES = [
    Profile(
        InputTemplate('mydata', PlainTextFormat,".fastq.gz",
            StaticParameter(id='encoding',name='Encoding',description='The character encoding of the file', value='utf-8'), 
            extension='.gz',
            multi=True
        ),                  
        InputTemplate('metadata', PlainTextFormat,".tsv",
            StaticParameter(id='encoding',name='Encoding',description='The character encoding of the file', value='utf-8'), 
            extension='.tsv',
            unique=True 
        ),
        #------------------------------------------------------------------------------------------------------------------------
        OutputTemplate('out',PlainTextFormat,'Name of output',
...
#
COMMAND = WEBSERVICEDIR + "/pema_wrapper.py $DATAFILE $STATUSFILE $OUTPUTDIRECTORY"

#Or for the shell variant:
#COMMAND = WEBSERVICEDIR + "/pema_wrapper.sh $STATUSFILE $INPUTDIRECTORY $OUTPUTDIRECTORY $PARAMETERS"
